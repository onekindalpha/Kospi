...
def make_plotly_chart(df: pd.DataFrame, market: str, sig: dict,
    ...
    fig.update_layout(
        ...
        yaxis=dict(
            title="지수", domain=[0.52, 1.0], range=y1_range,
...
def main():
    ...
    st.caption("GitHub에 저장된 데이터를 읽어 표시합니다.")

    # ── 사이드바 ──────────────────────────────────────
    with st.sidebar:
        st.header("⚙️ 설정")
        market = st.selectbox("마켓", ["KOSPI", "KOSDAQ"])
        fetch_btn = st.button("🔄 GitHub 데이터 새로고침", type="primary", width='stretch')
    ...
    # NH-NL은 로컬 캐시 우선, 없으면 GitHub CSV 사용
    nhnl_df = None
    local_nhnl = _nhnl_cache_path(market, df["date"].iloc[-1] if not df.empty else datetime.today().strftime("%Y%m%d"))
    if local_nhnl.exists():
        try:
            nhnl_df = pd.read_csv(local_nhnl, dtype={"date": str})
            if not nhnl_df.empty:
                nhnl_df["dt"] = pd.to_datetime(nhnl_df["date"].astype(str), format="%Y%m%d", errors="coerce")
                nhnl_df = nhnl_df.dropna(subset=["dt"]).sort_values("dt").reset_index(drop=True)
        except Exception:
            nhnl_df = None
    ...
    # ══════════════════════════════════════════════
        ...
        if nhnl_df is None or nhnl_df.empty:
            st.warning("NH-NL CSV를 찾지 못했습니다.")
        else:
            from plotly.subplots import make_subplots as _msp2

            nhnl_df = nhnl_df.copy()
            nhnl_df["dt"] = pd.to_datetime(nhnl_df["date"].astype(str), format="%Y%m%d", errors="coerce")
            nhnl_df = nhnl_df.dropna(subset=["dt"]).sort_values("dt").reset_index(drop=True)
            ...
            idx_week = df.copy()
            idx_week["dt"] = pd.to_datetime(idx_week["date"].astype(str), format="%Y%m%d", errors="coerce")
            idx_week = idx_week.dropna(subset=["dt"])
            idx_week = idx_week[["dt", "close"]].sort_values("dt").set_index("dt").resample("W-FRI").last().dropna().reset_index()
            idx_week = idx_week[(idx_week["dt"] >= pf3["dt"].min()) & (idx_week["dt"] <= pf3["dt"].max())].reset_index(drop=True)

            last_row = pf3.iloc[-1]
            c1, c2, c3 = st.columns(3)
            c1.metric("주간 새 고점 수", f"{int(last_row['new_highs']):,}" if "new_highs" in pf3.columns and pd.notna(last_row.get("new_highs")) else "N/A")
            c2.metric("주간 새 저점 수", f"{int(last_row['new_lows']):,}" if "new_lows" in pf3.columns and pd.notna(last_row.get("new_lows")) else "N/A")
            ...
            fig_hl.add_trace(go.Scatter(
                x=idx_week["dt"], y=idx_week["close"].astype(float),
            ...
            fig_hl.update_layout(
                ...
                yaxis=dict(title="지수"),
                yaxis2=dict(title="고점-저점 수치", zeroline=True, zerolinecolor="rgba(150,150,150,0.4)"),
            )
